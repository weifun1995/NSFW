...
class GitHub {
    ...
    async validateGitHubToken() {
        try {
            // 测试令牌基本信息
            console.log('当前使用的令牌前缀：', GITHUB_SECRET_KEY?.substring(0, 4) + '...')
    ...
    async uploadFileToGitHub(filename, filedata) {
        ...
        try {
            ...
            // 使用 octokit.rest.repos.createOrUpdateFileContents 替代 request
            const response = await octokit.rest.repos.createOrUpdateFileContents({
                owner,
                repo,
                path: fullPath,
                message: fileSha ? `更新文件 ${filename}` : `创建文件 ${filename}`,
...
const uploadFileToGitHub = new GitHub().uploadFileToGitHub
